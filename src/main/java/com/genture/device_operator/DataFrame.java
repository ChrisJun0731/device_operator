 ...
 */
class DataFrame {
    private byte start_byte = (byte)0xaa;
    private byte[] address_byte = {(byte)0xff, (byte)0xff};
    private byte end_byte = (byte)0xcc;
     ...
     * @return
     */
    public byte[] createDataFrame(int cmd, byte... data){
        if(data != null){
            byte[] data_prefix = {start_byte, address_byte[0], address_byte[1], (byte)cmd};
            byte[] data_suffix = convertByteData(data);
            byte[] data_frame_no_crc = new byte[data_prefix.length+data_suffix.length+1];
            System.arraycopy(data_prefix,0, data_frame_no_crc, 0, data_prefix.length);
            System.arraycopy(data_suffix, 0, data_frame_no_crc, data_prefix.length, data_suffix.length);
            data_frame_no_crc[data_frame_no_crc.length-1] = end_byte;
            byte[] crc = getCRCArray(data_frame_no_crc);
            byte[] data_frame = new byte[data_frame_no_crc.length + 2];
            System.arraycopy(data_frame_no_crc, 0, data_frame,0, data_frame_no_crc.length);
            System.arraycopy(crc, 0, data_frame, data_frame_no_crc.length, 2);
            return data_frame;
        }
        else{
            byte[] data_frame_no_crc = {start_byte, address_byte[0], address_byte[1], (byte)cmd, end_byte};
            byte[] crc = getCRCArray(data_frame_no_crc);
            byte[] data_frame = new byte[data_frame_no_crc.length + crc.length];
            System.arraycopy(data_frame_no_crc, 0, data_frame, 0, data_frame_no_crc.length);
            System.arraycopy(crc, 0, data_frame, data_frame_no_crc.length, crc.length);
            return data_frame;
        }
    }
     ...
     * @return
     */
    public byte[] getCRCArray(byte[] data_frame_no_crc){
        Util util = new Util();
        int result = Pub_CalcCRC(data_frame_no_crc, data_frame_no_crc.length);
        util.int2buf(result, crc, 0);
        return crc;
    }
     ...
     * @return
     */
    public int Pub_CalcCRC(byte[] dataFrame, int len){
        int fcs = 0xffff;
        for(int i=0; i<len; i++){
     ...
     * @return
     */
    public byte[] convertByteData(byte[] data){
        int extraSpace = 0;
        int prior_final = 0;
        for(int i=0; i<data.length; i++){
            if(data[i] == (byte)0xaa|| data[i] == (byte)0xcc|| data[i] == (byte)0xee){
                ++extraSpace;
            }
        }
        byte[] finalData = new byte[data.length+extraSpace];
        for(int i=0; i<data.length; i++){
            if(data[i]==(byte)0xaa){
                finalData[prior_final++] = (byte)0xee;
                finalData[prior_final++] = (byte)0x0a;
            }
            else if(data[i]==(byte)0xcc){
                finalData[prior_final++] = (byte)0xee;
                finalData[prior_final++] = (byte)0x0c;
            }
            else if(data[i]==(byte)0xee){
                finalData[prior_final++] = (byte)0xee;
                finalData[prior_final++] = (byte)0x0e;
            }
            else{
                finalData[prior_final++] = data[i];
            }
        }
        return finalData;
    }
